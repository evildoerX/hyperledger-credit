# 房产征信
## 项目背景
房地产经纪⾏业从业⼈员存在以下特点：从业⼈员基数⼤，流动性强。针对以上两点问题，⽤户希望建⽴⼀个⼈员管理
信⽤体系，来规范业内从业⼈员的市场⾏为。具体包括
1. 记录⾏业内经纪⼈的从业经历
2. 记录⾏业内经纪⼈的违规⾏为
3. ⼤众可以从系统中根据特定信息查询某个经纪⼈的以上两点记录
鉴于房地产⾏业协会政府⼒量有限，⽤户也不希望⼀下进⾏⾏业内的强制性改⾰，所以希望能将产品进⾏项⽬周期迭
代，逐步推动这个诚信体系的完善。具体迭代关系如下：
- 1.0版本(当前版本)：主要功能是⾏业⿊名单，使⽤⻆⾊是各个企业的HR。企业管理⼈可以记录企业内⿊名单员⼯
的信息登记上区块链。⾏业内部⼈员（例如HR）在招聘员⼯时可以查看⿊名单经纪⼈的从业经历和曾经有的违规
⾏为，避免恶性从业⼈员轻易跳槽⼆次就业，扰乱市场顺序。
- 2.0版本：开放经纪⼈端功能，实现经纪⼈登录系统维护⾃⼰的基本信息，查看⾃⼰的不良⾏为记录和优秀⾏为嘉
奖。
- 3.0版本：开放公众C端，⽀持⽤户通过经纪⼈持牌⼆维码扫码查询为⾃⼰服务的经纪⼈的市场⾏为记录。接⼊更多
社会诚信系统，增加房产诚信系统数据来源和评估纬度
当前系统处于1.0版本阶段，若⽆特殊说明，后续所有描述皆为当前版本
...
### 加盟的门店与经纪
1. 管理员：自己门店和行业的相关报表
1. HR：主要处理业务包含员工管理，投诉，违规管理，上岗证申请等
2. 业务员: 上岗证缴费，查看个人诚信数据，和申诉
### 协会的角色关系
...
(诚信管理) ..> (投诉管理): <<include>>
(投诉管理) ..> (添加投诉): <<include>>
(添加投诉) ..> (添加经纪人投诉): <<include>>
(添加投诉) ..> (添加信访投诉): <<include>>
(添加投诉) ..> (添加经纪公司投诉): <<include>>
(投诉管理) ..> (获取投诉列表): <<include>>
(投诉管理) ..> (获取投诉详情): <<include>>
(投诉管理) ..> (更新投诉信息): <<include>>
...
(黑名单管理) ..> (审核黑名单): <<include>>
(业务管理员) ..> (上岗证管理): <<include>>
(上岗证管理) ..> (添加上岗证申请单): <<include>>
(上岗证管理) ..> (审核上岗证申请单): <<include>>
(上岗证管理) ..> (申请上岗证): <<include>>
(上岗证管理) ..> (审核上岗证): <<include>>
...
(业务管理员) ..> (云文件管理): <<include>>
(云文件管理) ..> (获取云文件列表): <<include>>
(云文件管理) ..> (查看云文件详情): <<include>>
(云文件管理) ..> (提取云文件): <<include>>
(云文件管理) ..> (转存云文件): <<include>>
(业务管理员) ..> (消息管理): <<include>>
(消息管理) ..> (获取消息列表): <<include>>
...
##### 基础流程
1. 协会管理员登录诚信系统
2. 协会管理员在添加机构（经纪公司）界面输入机构信息
3. 协会管理员点击添加按钮添加机构
##### 辅助流程
1. 协会管理员点击链上机构select 获取链上机构
##### 异常流程
#### 获取机构列表
##### 参与角色
协会管理员
